...
- **Widget:** key `"pi-todo"`, placement `aboveEditor`, factory form, register-once + `requestRender()`; renders iff toggle ON **and** `tasks.length > 0`; max 10 lines incl. header, overflow → `+N more`; ordering pending → in_progress → completed, stable by id within status; header `Todos <done>/<total>` plus `(<k> in progress)` when k > 0.
...
export function formatWidget(tasks: Task[], maxLines: number): string[];
```

`formatWidget` returns: header `Todos <done>/<total>` (plus `  (<k> in progress)` when `k > 0`), then task lines ordered pending → in_progress → completed (stable by id), capped at `maxLines` total lines with a final `+N more` line when overflow.

- [ ] **Step 1: Write the failing tests** (append to `test/core.test.ts`)

```ts
import { formatList, formatTaskLine, formatWidget, MARKER_PENDING, MARKER_ACTIVE, MARKER_DONE, markerFor, type Task } from "../src/core.js";
...
describe("markers and formatting", () => {
  ...
  it("formatWidget orders pending, in_progress, completed by id", () => {
    const tasks = [
      task(3, "done3", "completed"),
      task(1, "open1", "pending"),
      task(2, "active2", "in_progress"),
    ];
    expect(formatWidget(tasks, 10)).toEqual([
      "Todos 1/3  (1 in progress)",
      "○ #1 open1",
      "▸ #2 active2",
      "✓ #3 done3",
    ]);
  ...
  it("formatWidget caps at maxLines with +N more", () => {
    const tasks = Array.from({ length: 12 }, (_, i) => task(i + 1, `t${i + 1}`, "pending"));
    const lines = formatWidget(tasks, 10);
  ...
  it("all rendered strings are printable ASCII plus the marker set", () => {
    ...
    const outputs = [formatList(tasks), ...formatWidget(tasks, 10), formatTaskLine(task(1, "A", "pending"))].join("\n");
...
Expected: FAIL — `formatTaskLine`/`markerFor`/`formatWidget` are not exported yet.
...
const STATUS_ORDER: Record<TaskStatus, number> = { pending: 0, in_progress: 1, completed: 2 };
...
export function formatWidget(tasks: Task[], maxLines: number): string[] {
  ...
  const header = `Todos ${done}/${tasks.length}` + (active > 0 ? `  (${active} in progress)` : "");
  const sorted = [...tasks].sort(byStatusThenId);
  const lines = [header];
  const budget = Math.max(1, maxLines - 1);
  const visible = sorted.slice(0, budget);
  const hidden = sorted.length - visible.length;
  for (const t of visible) lines.push(formatTaskLine(t));
...
`renderWidgetLines` = `formatWidget` result with marker coloring: `▸` wrapped in `theme.fg("accent", marker)`, `✓` in `theme.fg("dim", marker)`, `○` unstyled; header line wrapped in `theme.fg("dim", headerText)` when no task is active and `theme.fg("accent", headerText)` when at least one is in_progress.
...
describe("renderWidgetLines", () => {
  it("colorizes markers and header per spec", () => {
    ...
    expect(lines[0]).toBe("<accent>Todos 1/3  (1 in progress)</accent>");
    expect(lines).toContain("○ #3 C");
    expect(lines).toContain("<accent>▸</accent> #1 A");
    expect(lines).toContain("<dim>✓</dim> #2 B");
  });
  ...
  it("honors maxLines", () => {
    ...
    expect(renderWidgetLines(state, EMPTY_THEME, 5).length).toBe(5);
  });
});
...
- **Type consistency:** `applyOp`/`TodoResult` produced in Task 2 are consumed identically in
  Tasks 4/6; `formatWidget` (Task 3) vs `renderWidgetLines` (Task 5) are distinct by design —
  the former is the plain text used by `list`, the latter adds theme colors for the widget.